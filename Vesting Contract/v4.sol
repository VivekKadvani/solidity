...
function withdraw(uint256 index) external {
    ...
    require(vesting.amount >= withdrawable, "Not enough tokens to withdraw");
...
function calculate_available_withdraw_token(uint256 index) public view returns(uint256) {
    ...
    uint256 total_avl_withdraw = (total_slice_count * vesting.recive_on_interval) - vesting.temp;
    calculate_remaining_token(index);
    if()
    require(total_avl_withdraw <= vesting.amount, "vesting complete");
    return total_avl_withdraw;
}

function calculate_remaining_token(uint index) public view returns(uint256){
    VestingSchedule storage vesting = vestings[msg.sender][index];
